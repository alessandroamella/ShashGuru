...
const isSpectator = computed(
	() => !liveState.value.is_free && !isController.value,
);
...
function addMove(move) {
	...
	try {
		...
		if (chessMove) {
			...
			const existingChild = currentNode.value.children.find(
				(child) => child.move === chessMove.san,
			);
			if (existingChild) {
				...
				if (currentNode.value.children.length === 0) {
					const newNode = currentNode.value.addChild(
						chessMove.san,
						chess.fen(),
					);
					...
					const newNode = currentNode.value.addVariation(
						chessMove.san,
						chess.fen(),
					);
					currentNode.value = newNode;
...
async function fetchEvaluationForNode(node) {
	...
	try {
		...
		const evaluation = await EvaluationService.fetchEvaluation(
			node.fen,
			depth,
			lines,
		);
...
function rebuildMovesDisplay() {
	...
	selectedMoveIndex.value =
		selectedPath.value.length > 0 ? selectedPath.value[0] : -1;
}
...
function promoteVariation(nodeToPromote) {
	...
	while (
		variationRoot.parent &&
		variationRoot.parent.mainLine === variationRoot
	) {
	...
	if (variationRoot.parent) {
		...
		console.log(
			`Successfully promoted move "${nodeToPromote.move}" to main line`,
		);
	}
...
function deleteMove(nodeToDelete) {
	...
	if (childIndex !== -1) {
		...
		console.log(
			`Successfully deleted move "${nodeToDelete.move}" and all subsequent moves`,
		);
